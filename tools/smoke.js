...
console.log('city mode');
{
  ...
  if (cityA.atmosphere.birds.length > 3) fail('city bird flock is too large');
  if (!cityA.atmosphere.clouds.length) fail('city atmosphere has no clouds');
  else ok('city atmosphere is sparse, seeded and bounded');
  const city1 = AD.city.generate('city-scale', { count: 24, cityScale: 1 });
  const city5 = AD.city.generate('city-scale', { count: 24, cityScale: 5 });
  const civicOccupied = AD.city.generate('civic-seed-0', { count: 48, cityScale: 2 });
  const civicPark = AD.city.generate('civic-seed-1', { count: 48, cityScale: 2 });
  if (civicOccupied.civic.active === civicPark.civic.active) fail('civic occupancy is not seed-random');
  if (!civicPark.parks.some(function (p) { return p.role === 'civic-park' && p.trees.length >= 3 && p.amenity; })) fail('empty civic space lacks functional park structure');
  if (new Set(civicPark.parks.map(function (p) { return (p.x1 - p.x0).toFixed(2) + ':' + (p.z1 - p.z0).toFixed(2); })).size < 2) fail('parks do not vary in size');
  else ok('civic spaces randomize between occupied cores and functional parks');
  const city384 = AD.city.generate('city-count', { count: 384, cityScale: 1 });
  if (city384.buildings.length < 200) fail('high building-count toggle did not produce far more buildings');
  else ok('high building-count toggle produces a large city');
